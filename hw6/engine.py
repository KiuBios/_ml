...
class Value:
    ...
    def sigmoid(self):  # sigmoid 的正向傳遞
        out = Value(1 / (1 + math.exp(-self.data)), (self,), 'Sigmoid')

        def _backward():  # sigmoid 的反向傳遞
            sigmoid_output = 1 / (1 + math.exp(-self.data))
            self.grad += sigmoid_output * (1 - sigmoid_output) * out.grad
        out._backward = _backward

        return out

    def exp(self):  # exp 的正向傳遞
        out = Value(math.exp(self.data), (self,), 'Exp')

        def _backward():  # exp 的反向傳遞
